...
| **AI Fluency** | Can she work where the field is going? | Try me block, two engineered-system cards, the client web roster, then the tool matrix | `src/data/ai.ts` |
...
- **`Try me` is inert on purpose.** The keyboard block at the top of AI Fluency is a `div`, not a link, and carries a `Coming soon` chip — the chatbot route does not exist. When `/ask` ships, that block and the desktop `?` both become links to it and the chip comes off.
