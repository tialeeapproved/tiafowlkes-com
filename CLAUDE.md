...
- **The window's title bar holds position too**, so the page you're in is always named even deep in a scroll. It runs denser than the window and carries its own blur, because content passes directly beneath it. `--stack-top` (62px) is what keeps the stacked cards clearing it — change one and check the other.
